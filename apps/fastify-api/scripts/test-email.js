#!/usr/bin/env node
/**
 * 简化的邮件发送测试脚本（JavaScript 版本）。
 *
 * @description 直接使用 nodemailer 测试邮件配置，不依赖 TypeScript 编译。
 * 使用方法：node scripts/test-email.js <recipient-email>
 *
 * @example
 * ```bash
 * # 测试发送邮件到指定邮箱
...
async function testEmail() {
  ...
  if (!recipientEmail) {
    ...
    console.log('使用方法: node scripts/test-email.js <recipient-email>');
    console.log('示例: node scripts/test-email.js test@example.com');
    process.exit(1);
  }
  ...
  if (!mailHost || !mailUsername || !mailPassword) {
    ...
    // 提供 QQ 邮箱配置示例
    if (recipientEmail.includes('@qq.com')) {
      console.error('💡 QQ 邮箱配置示例:');
      console.error('   MAIL_HOST=smtp.qq.com');
      console.error('   MAIL_USERNAME=your-email@qq.com');
      console.error(
        '   MAIL_PASSWORD=your-authorization-code  # 必须使用授权码，不是QQ密码',
      );
      console.error('   MAIL_PORT=587');
      console.error('   MAIL_SECURE=false');
      console.error('');
      console.error(
