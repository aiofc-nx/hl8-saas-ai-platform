 ...
 * 使用方法：node scripts/test-email-simple.js <recipient-email>
 *
 * @example
 * ```bash
 * # 测试发送邮件到指定邮箱
 * node scripts/test-email-simple.js test@example.com
 * ```
 */
...
// 获取当前文件目录（CommonJS 兼容方式）
// 使用 process.cwd() 作为基础路径，因为脚本从项目根目录运行
const __dirname = path.resolve(process.cwd(), 'apps/fastify-api/scripts');
...
async function testEmail() {
  ...
  if (!recipientEmail) {
    ...
    console.log('示例: node scripts/test-email-simple.js test@example.com');
    process.exit(1);
  }
