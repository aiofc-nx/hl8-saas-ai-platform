...
const SignUpForm = () => {
  ...
  const {
    execute,
    isExecuting,
    result: { validationErrors, serverError, data },
  } = useAction(signUpWithCredentials);

  // 注册成功后自动跳转到邮箱确认页面
  useEffect(() => {
    if (data && !serverError && !isExecuting) {
      const redirectTo =
        (data as { redirectTo?: string })?.redirectTo || '/auth/confirm-email';
      router.push(redirectTo);
    }
  }, [data, serverError, isExecuting, router]);
