...
// Logger 是 PinoLoggerService 的别名，从 index.ts 导出
// 在运行时，Logger 就是 PinoLoggerService 类本身
// 我们需要创建一个提供者，使 Logger 可以作为 token 使用
// 由于 Logger 在运行时就是 PinoLoggerService，我们可以使用 useExisting
// 但需要从 index.ts 导入 Logger 作为 token（避免循环依赖，使用延迟导入）
...
export class PinoLoggingModule {
  ...
  static forRoot(options?: PinoLoggerModuleOptions): DynamicModule {
    ...
    return {
      ...
      // 导出 PinoLoggerService，Logger 可以通过类型别名自动解析
      // 注意：由于 Logger 在运行时就是 PinoLoggerService 类，我们只需要导出 PinoLoggerService
      // 当其他模块使用 @Inject(Logger) 时，NestJS 会自动使用 PinoLoggerService 实例
