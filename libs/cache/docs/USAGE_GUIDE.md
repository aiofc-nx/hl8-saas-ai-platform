...
> **注意**: 写路径控制器功能当前尚未实现。如需远程触发缓存失效或预热，建议直接注入 `CacheConsistencyService` 或 `CacheNotificationService` 在业务层实现。
...
```ts
const cacheKey = this.tenantKeyBuilder.build({
  tenantId,
  configKey: 'feature',
});
...
> **注意**: HTTP API 控制器功能当前尚未实现。如需远程触发缓存失效或预热，建议通过业务层直接调用 `CacheConsistencyService` 或 `CacheNotificationService`。
...
| 测试覆盖率  | 运行测试覆盖率报告，当前整体覆盖率 82.7%                 | `pnpm --filter @hl8/cache test:cov` |
| Lint & 格式 | 保证 TSDoc、中文注释与规范一致                           | `pnpm --filter @hl8/cache lint`     |
...
通过本指南，你可以快速搭建缓存读写链路，并在写操作后可靠地失效与预热缓存，满足多租户、高并发场景下的数据一致性要求。如遇问题，可参考 FAQ 或直接查看 `libs/cache/src` 中的源码与测试案例。

**测试覆盖率说明**：

- 当前库的整体测试覆盖率为 **82.7%**（语句覆盖率）
- 所有核心服务的测试覆盖率均达到或超过 **80%**
- 测试文件位于 `libs/cache/src/**/*.spec.ts`
