...
// 在 monorepo 中，依赖通常在根目录的 node_modules 中
// 尝试从根目录创建 require，如果失败则使用包目录
let requireModule: ReturnType<typeof createRequire>;
try {
  // 尝试从根目录创建 require（monorepo 场景）
  const rootPackageJson = path.resolve(process.cwd(), 'package.json');
  if (existsSync(rootPackageJson)) {
    requireModule = createRequire(rootPackageJson);
  } else {
    requireModule = createRequire(path.resolve(packageRootDir, 'package.json'));
  }
} catch {
  requireModule = createRequire(path.resolve(packageRootDir, 'package.json'));
}
